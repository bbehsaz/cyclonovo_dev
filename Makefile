...
ifdef print_spectrum_help
	...
	@echo "print_spectrum not in PATH. NPDTools binaries must be in PATH.";	exit 1;
endif
ifdef print_score_help
	@echo "print_score found."
else
	@echo $(print_spectrum_help)
	@echo "print_score not in PATH. NPDTools binaries must be in PATH.";	exit 1;
endif
	@echo "Running CycloNovo tests ..."
	# testsurugsamide:
	cp ./scripts/cyclonovo.py cyclonovo
	chmod a+wrx cyclonovo
	@echo "Running tests surugamdie";	./cyclonovo -s data/surugamide_spectrum.mgf --denovo -o surugamide_test --monomers standard -e 0.015 -d 0.015 --verbosity --kmer_threshold 6
