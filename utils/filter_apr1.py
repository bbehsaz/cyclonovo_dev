...
def generateAllAutconv(
    ...
    for dis in sorted(ALLcleanedAutoconvolutions.items(), key=itemgetter(1), reverse=True):
        if dis[1] > 0:
            ...
            finalClusters[distance] = (dis[1], round(maxDis - minDis, 3) / 2.0, minDis, maxDis,
                                       all_pair_peaks)  # tuple with the frequency as the first component and the diameter of the cluster as second
    ...
    for distance1 in sortedConvolutios:
        ...
        if currentVal >= thresholdValue:
            for mass in set(standardAminoMasses):
                ...
                if (mass - distance) < (e) and ((abs(mass - min_distance) < e) or (abs(mass - max_distance) < e) or (
...
def generate_convolutions(
    ...
    constituentMonoMers = []
    finalClusters, ALLcleanedAutoconvolutions, precursorMass, charge = generateAllAutconv(peaks, e, thresh,
                                                                                          representative,
                                                                                          standardAminoMasses,
...
def output_cyclopeptide_polymers(
        ...
        precursor, retention, lines, writeOriginalSpectra, num_peaks, peptide, alpha, betta, thresholdValue,
        stand_aa_peak_pairs):
    totalCyclopeptide = 0
    ...
    if numStnd > -1:
        ...
        if len(final_STNDconvolutions) == 2:  # checking if this is polymer of a single amino acid
            found_chains = 0
            chains = []
        ...
        if len(final_STNDconvolutions) > 2 and not polymer:
            totalCyclopeptide += 1
            compound = "cyclopeptide"
        elif len(final_STNDconvolutions) > 2 and polymer:
            found_chains = 0
            n = 0
            chains = []
