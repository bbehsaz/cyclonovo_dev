...
def output_denovo_results(candidatePeptides, precursorMass, retention, charge, peptide, benchmark_file, kmerSize,
    ...
    arguments_to_print = [(kmerSize, kmerThreshold), precursorMass, retention, charge, len(peptidesUptoCorrectScore),
                          len(scores), maxScore]
...
def matchAllKmers(foundAutoConv, k, peaks, e):
    ...
    allMatches = {}
    protonMass = 1.00728
    for x in __combinations_with_replacement(foundAutoConv, k):
        elements = x
...
def generateAllKmerLinearScore(allMatches, combination, k, precursorMass):
    ...
    for compos in __combinations_with_replacement(combination, k):
        for x in itertools.permutations(list(compos), k):
            ...
            if kmerName in allKmersScore:
                continue
            notPossible = False
            kmerFrequencies[kmerName] = dict((a, kmer.count(a)) for a in aminoFrequencies.keys())
...
def generate_kmers_score(combination, kmerSize, pepMass, peaks, e):
    ...
    def extend_to_k(k, kmerScores, kmerFrequencies,
                    kmerSequences):  # all-kmers with their linear scores using the k-1-mers in the kmers_scores
        kmerScores[k] = {}
...
def find_feasible_combintions(foundAutoConv, l, peptideMass, delta):
    ...
    import itertools
    protonMass = 1.00728
    for combination in __combinations_with_replacement(foundAutoConv, l):
...
def find_aa_for_denovo(
        ...
        if currentVal > thresholdValue:
            for mass in set(standardAminoMasses):
                if mass in polymer_repeat_units:
                    if abs(mass - distance) < e and (
                            (abs(mass - min_distance) < e) or (abs(mass - max_distance) < e) or (
                            min_distance < mass and mass < max_distance)):
                        for pair in peak_pairs:
                            polymerPeaks.add(pair[0])
                            polymerPeaks.add(pair[1])
                    continue

                if abs(mass - distance) <= 2 * e and (
                        (abs(mass - min_distance) < e) or (abs(mass - max_distance) < e) or (
                        min_distance < mass and mass < max_distance)):
    ...
    totalstd = 0
    import math
    return numSTDFound, N, topNconvolutions, standardAutconvCleaned, int(
        math.ceil(thresholdValue)), stand_aa_peak_pairs, polymerPeaks
